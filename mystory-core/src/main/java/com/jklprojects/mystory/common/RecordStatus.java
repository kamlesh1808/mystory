 ...
 */
public enum RecordStatus implements IStatus {
	NEW(1, "New"), PENDING_REVIEW(2, "Pending Review"), ACTIVE(3, "Active"), INACTIVE(4, "Inactive"), PURGED(5,
			"Purged"), REJECTED(6, "Rejected");

	private static final String SIMPLE_NAME = RecordStatus.class.getSimpleName();

	private final int id;
	private final String statusName;

	RecordStatus(int id, String statusName) {
		this.id = id;
	 ...
	 */
	public static RecordStatus toEnum(int id) {
		return Arrays.stream(RecordStatus.values()).filter(e -> e.getId() == id).findFirst().get();
	}
