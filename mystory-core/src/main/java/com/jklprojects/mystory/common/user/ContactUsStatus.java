 ...
 * @version 1, 2015-07-29
 * @version 2, 2018-06-23
 * @version 4, 2019-03-23
 */
public enum ContactUsStatus implements IStatus {
	NEW(1, "New"), REPLIED(2, "Replied");

	private static final String SIMPLE_NAME = ContactUsStatus.class.getSimpleName();

	private final int id;
	private final String statusName;

	ContactUsStatus(int id, String statusName) {
		this.id = id;
	 ...
	 */
	public static ContactUsStatus toEnum(int id) {
		return Arrays.stream(ContactUsStatus.values()).filter(e -> e.getId() == id).findFirst().get();
	}
